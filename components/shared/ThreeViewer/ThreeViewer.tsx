    ...
    <group ref={group}>
      {[...Array(12)].map((_, i) => {
        ...
        return (
          <group key={i}>
            {/* First strand */}
            <mesh position={[Math.cos(t) * radius, height, Math.sin(t) * radius]}>
              <sphereGeometry args={[0.08, 8, 8]} />
              <meshStandardMaterial color={helixColor} emissive={helixColor} emissiveIntensity={0.5} />
            </mesh>
            ...
            {i % 2 === 0 && (
              <mesh position={[0, height, 0]} rotation={[0, 0, Math.PI / 2]}>
                ...
                <meshStandardMaterial color={helixColor} opacity={0.6} transparent />
              </mesh>
            )}
          </group>
        );
...
export const ThreeViewer = () => {
  ...
  return (
    <Canvas
      ...
      performance={{ min: 0.5 }}
    >
      <color attach="background" args={[theme.isDarkMode ? "#000000" : "#ffffff"]} />
